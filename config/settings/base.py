...
from apps.accounts.constants import AccountURLConstants
...
DEFAULT_APPS = [
    'apps.accounts.apps.AccountsConfig',  # must be on top
...
USE_PAYMENT_OPTIONS = env('USE_PAYMENT_OPTIONS', default=False)
...
if USE_PAYMENT_OPTIONS:
    try:
        ...
        raise ImproperlyConfigured("Incorrect payment gateway setup")

# CELERY BROKER CONFIG
USE_CELERY_REDIS = env('USE_CELERY_REDIS', default=False)
...
if USE_CELERY_REDIS:
    try:
        ...
        raise ImproperlyConfigured("Incorrect Celery Redis setup")

# MAILCHIMP INTEGRATION
USE_MAILCHIMP = env('USE_MAILCHIMP', default=False)
if USE_MAILCHIMP:
